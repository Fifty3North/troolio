...
public class AllShoppingListsActor : StatefulActor<AllShoppingListsState>, IAllShoppingListsActor
{
    ...
    public IEnumerable<Event> Handle(AddShoppingList command)
    {
        ...
        return new[] { new ShoppingListAdded(command.Headers, command.ListId, joinCode) };
    }
    public IEnumerable<Event> Handle(JoinListUsingCode command)
    {
        if (command.Code == null || !State.Lists.ContainsKey(command.Code))
        {
            throw new InvalidJoinCodeException();
        }

        yield return new ListJoinedUsingCode(command.Headers, command.Headers.UserId, State.Lists[command.Code]);
    }
    #endregion
    ...
    public void On(ShoppingListAdded ev) => State = State with { Lists = State.Lists.Add(ev.JoinCode, ev.ListId) };
    #endregion
