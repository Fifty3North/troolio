...
.shoppingLists
    ...
    .data
        ShoppingList(v-for="list in ShoppingLists" :shoppingList="list" @add="AddItemToList(list.id,$event)" @check="CheckItem(list.id,$event)")
</template>
<script lang="ts">
export default {
    name:'ShoppingLists'
...
function CreateNewShoppingList(){
    ...
    let userId =  localStorage.getItem(LocalVariables.UserId) as string;
    let deviceId =  localStorage.getItem(LocalVariables.DeviceId) as string;
    let headers = {
        'userId': userId,
        'deviceId': deviceId
    }
    let shoppingListId = Guid.create().toString();
    let payload = {'title': ShoppingListName.value}
    
    axios.post(`${metaEnv.VITE_API_URL}ShoppingList/${shoppingListId}/CreateNewList`,payload, { headers }).then((response: any) => {
      if(response && response.status === 200){
          let newShoppingList:Interfaces.ShoppingList ={
            id:shoppingListId,
            title:payload.title,
            ownerId:userId,
            collaborators:[],
...
function AddItemToList(listId:string,emitPayload:Interfaces.AddToShoppingListEmit){
    ...
    let payload:Interfaces.AddToShoppingListPayload = {
    ...
    axios.post(`${metaEnv.VITE_API_URL}ShoppingList/${listId}/AddItemToList`,payload, { headers }).then((response: any) => {
      if(response && response.status === 200){
        let newShoppingList:Interfaces.ShoppingListItem ={
            id:payload.id,
            crossedOff:false,
